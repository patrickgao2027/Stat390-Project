...
% NeurIPS 2026 template (4 pages).
%
% Requires neurips_2026.sty next to this file.
%
% Compile: pdflatex final_report.tex   (twice to resolve refs)

\documentclass{article}

% Drop [final] for the submission build (the package switches on line numbers).
...
\begin{abstract}
I present a 36-iteration AutoResearch loop that produced a calibrated
...
test-time augmentation, an additive operating-point safety margin, and
best-of-$N$ weight-save for deployment --- achieves mean ROC-AUC
$0.900 \pm 0.005$ and mean recall $0.958 \pm 0.019$ across five
independent training replicates, with four of five replicates individually
clearing the 95\% recall target. Both pre-registered criteria
(AUC $\ge 0.85$, recall $\ge 0.95$) are met. The architecture sequence
(LR $\to$ AlexNet $\to$ EfficientNet-B0/B2/B4) was driven by the previous
baseline's measured failure mode, not pre-selected. We characterize which
loop directions produced stable value (architecture upgrades and threshold
calibration), which produced noise or regression (post-hoc determinism,
multiplicative safety margins, calibration-target tuning), and the
structural variance floor imposed by the frozen data pipeline. The
deployed checkpoint (iter 35 weights) reproduces its metrics bit-for-bit
on every inference (AUC 0.9021, recall 0.9676) and has been shipped
end-to-end through PyTorch~$\to$~ONNX~$\to$~TFLite to an Android
(Kotlin/CameraX/NNAPI) screening app for Samsung S22+.
\end{abstract}
...
\textbf{From topic to contract.} My \texttt{program.md} prompt asked for
...
This was formalized into an AutoResearch contract with three
...
\textbf{Deployment artifact.} Weights are saved to \texttt{model\_checkpoint.pt}
only when \texttt{raw\_threshold} beats the previous best (best-of-$N$
selection); loading this checkpoint skips training and yields deterministic
inference in $\approx 0.75$\,s. The checkpoint is also exported
PyTorch~$\to$~ONNX~$\to$~TFLite (fp16) and bundled into an Android app
with bit-for-bit output agreement.

\section{Experimental protocol}

\textbf{Loop design.} 36 iterations were executed sequentially: name one
variable to change in \texttt{model.py}, run \texttt{python -u run.py},
append one row to \texttt{results.tsv}, decide keep/discard/replicate.
From iter 14 onward, iterations were grouped in pre-declared 3-rep blocks
before any claim about a variable's effect could be made.
...
\begin{table}[h]
\caption{Locked configuration (iters 32-36): EfficientNet-B4 + two-phase
fine-tune + holdout threshold calibration + 4-view TTA + additive
SAFETY\_MARGIN=0.10 + best-of-$N$ weight save. Each row is one
...
35 & 0.9021 & \textbf{0.9676} & 0.7120 & 0.3805 \\
36 & 0.8919 & \textbf{0.9670} & 0.6811 & 0.3562 \\
\midrule
mean ($n=5$) & \textbf{0.9003} & \textbf{0.9584} & 0.7139 & 0.3829 \\
...
Both pre-registered targets are met on the 5-rep mean
(Table~\ref{tab:final}); \textbf{4 of 5 individual reps} cross the recall
target. The deployed checkpoint (iter 35 weights, saved by best-of-$N$
selection on \texttt{raw\_threshold}, verified by reloading at
\texttt{results.tsv} row 37) produces AUC 0.9021 and recall 0.9676 every
time it is reloaded --- bit-for-bit identical metrics across reloads, in
0.75~s end-to-end vs $\approx$5500~s for training. The deployed model
meets the recall target deterministically.

\subsection{Stable directions (what produced real value)}

Four single-variable changes account for essentially all of the gain
from the iter-1 baseline (AUC 0.793, recall 0.898).
...
(0.825 vs 0.850). The shuffle in \texttt{prepare.py} is in fact seeded
(\texttt{ds.shuffle(seed=RANDOM\_STATE)}); the real culprits are the
five \texttt{tf.image.random\_*} augmentation ops inside
\texttt{.map(augment, num\_parallel\_calls=AUTOTUNE)}, none of which
receive an op-level \texttt{seed=} argument, combined with the
non-deterministic ordering of \texttt{AUTOTUNE} parallel map workers.
\texttt{tf.random.set\_seed()} in \texttt{model.py} cannot pin per-op
random state for ops whose graph was already constructed inside
\texttt{prepare.py}. \emph{Determinism is unreachable from the editable
surface alone.} This is the most consequential finding of the loop: it
converts training-time variance from a tunable parameter into a structural
constant, and motivated the eventual best-of-$N$ deployment artifact.
\texttt{prepare.py} was not edited to add seeding to the augmentation
operations in order to keep consistency across the AutoResearch
iterations.
...
uses unseeded \texttt{tf.image.random\_*} augmentations under
\texttt{num\_parallel\_calls=AUTOTUNE}, per-run training variance is a
structural constant --- no \texttt{model.py}-side change can lower it.
The locked recall std of 0.019 (Table~\ref{tab:final}) is at this floor.
Individual training runs are therefore not bit-exactly reproducible;
the deployment checkpoint (\texttt{model\_checkpoint.pt} + TFLite) is,
and is what we ship.

\textbf{Test set used only for generalization measurement.} The test set
was never used to train or tune the model. All hyperparameter decisions
used the per-iteration holdout cal slice; test metrics were logged solely
to measure generalization.

\textbf{Pipeline ceiling.} 128$\times$128 resolution and 30\% training
subsample are characteristics of the frozen pipeline; a higher ceiling
exists at full data and native resolution but cannot be tested without
invalidating the experimental record.

\section{What the AutoResearch loop did well and poorly}

The loop's biggest strength was enforcing single-variable iteration:
every block from iter 14 onward is a clean controlled experiment
readable directly from \texttt{results.tsv}, with negative results
preserved and labeled rather than discarded.

The loop's main weakness was early-block confounding: iters 4, 5, and 7
each changed two variables at once, producing the project's largest
single AUC jump (0.79 $\to$ 0.90) without a clean causal account.
Four retroactive iterations (10-13) were spent untangling what actually
drove the gain --- compute that a pre-flight diff check would have saved
entirely.

In my experience, auto-research is extremely good at generating new
ideas and greatly accelerating the coding process toward set-out goals.
The loop produced more candidate levers (\texttt{pos\_weight} tuning,
threshold estimators, TTA configurations, \texttt{SAFETY\_MARGIN}
variants, calibration-batch counts) than a human iterator working alone
would have proposed in the same wall-clock window, and the mechanical
parts --- edit one constant, launch the run, log the row, draft the
commit --- were compressed to a few minutes of human time per
iteration. Both target metrics (AUC $\ge 0.85$, recall $\ge 0.95$) were
met and the deployment artifact shipped within the project window,
which is the outcome the loop was set up to produce.
